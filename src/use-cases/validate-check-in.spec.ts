...
describe('Check In Use Case', () => {
  ...
  it('Should not be able to validate a check-in after 20 minutes of its creation', async () => {
    ...
    const maxCheckInValidationTimeInMilliseconds =
      1000 * 60 * env.MAX_CHECK_IN_VALIDATION_TIME_IN_MINUTES

    const minutesInMilliseconds = 1000 * 60

    vi.advanceTimersByTime(
      maxCheckInValidationTimeInMilliseconds + minutesInMilliseconds,
    )
